...
public:
        ...
        const astra::CoordinateMapper& mapper)
    {
        if (depthFrame.is_valid())
        {
            ...
            //DeltaZ is height 
            int DeltaZ = 636 - middle;

            //Logic for resetting buffers 
            if (!(DeltaZ >= 24)) {
                islargebuffer = 0;
            }            
            if (!(DeltaZ < 24)) {
            ...
            //Only execute when Height is greater than 0
            if (DeltaZ > 0) {
                //std::cout << " Height: " << DeltaZ;
                if (DeltaZ >= 24) {
                    if (islargebuffer == 2) {
                        std::cout << " This is a Large Object!" << std::endl;
                    }
                    islargebuffer++;
                }
                if (DeltaZ < 24) {
                    if (issmallbuffer == 2) {
                        std::cout << " This is a Small Object!" << std::endl;
